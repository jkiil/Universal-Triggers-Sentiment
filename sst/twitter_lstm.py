...
class TwitterDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        for row in data:
            ...
            # Convert to 0 = negative, 1 = positive
            if label == 2:
                continue
            ...
            tokens = self._tokenizers['tokens'].tokenize(row['text'])
            label = str(label)
...
def main(dataset_label_filter, test_triggers, model_no_str):
    ...
    model_path = "tmp\\" + EMBEDDING_TYPE + "_twitter_" + dataset_label_filter + "_" + model_no_str + "model.th"
